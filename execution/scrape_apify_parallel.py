...
def run(payload, token_data=None, slack_notify=None):
    """
    Modal procedural entry point.
    """
    mode = payload.get("mode", "leads")
    
    if mode == "competitors":
        industry = payload.get("industry")
        location = payload.get("location")
        exclude = payload.get("exclude")
        max_comp = payload.get("max", 3)
        
        if not industry or not location:
            return {"status": "error", "error": "Missing industry or location for competitor research"}
            
        if slack_notify:
            slack_notify(f"🕵️ *Competitor Research:* `{industry}` in `{location}`")
            
        try:
            results = scrape_competitors(industry, location, exclude, max_comp)
        ...
        location = payload.get("location")
        total_count = payload.get("total_count", 50)
        
        if not query or not location:
            return {"status": "error", "error": "Missing query or location for lead scraping"}
            
        if slack_notify:
            slack_notify(f"🚀 *Parallel Scraping:* `{query}` in `{location}`")
            
        try:
            results, total_time, _ = scrape_parallel(
                query=query,
                location=location,
                total_count=total_count,
                strategy=payload.get("strategy", "regions"),
                num_partitions=payload.get("partitions", 4),
                require_email=not payload.get("no_email_filter", False)
            )
            return {
                "status": "success", 
