...
def analyze_google_my_business(business_name, location):
    ...
    run_input = {
        ...
        "language": "en",
        "maxReviews": 10,  # Get review stats (replaced deprecated includeReviews)
    }
...
def run(payload, token_data=None, slack_notify=None):
    ...
    if not business:
        return {"status": "error", "error": "Missing 'business' in payload"}

    if slack_notify:
        slack_notify(f"🔍 *Social Analysis:* Examining `{business}` in `{location}`")

    try:
        results = analyze_social_presence(business, location)
        return {
            "status": "success",
            "data": results
        }
